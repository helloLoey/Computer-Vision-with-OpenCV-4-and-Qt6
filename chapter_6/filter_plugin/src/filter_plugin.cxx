...
void Filter_Plugin::setupUi(QWidget *t_parent)
{
    ui = new Ui::PluginGui;
    ui->setupUi(t_parent);
...
void Filter_Plugin::processImage(const cv::Mat &t_source, cv::Mat &t_destination)
{
    using namespace cv;
    Matx33f f2dkernel;
    ...
    case DERIVATIVES_PAGE:
        if(ui->derivSobelRadio->isChecked())
            Sobel(t_source, t_destination, -1, 1, 1, 3, ui->derivScaleSpin->value(), ui->derivDeltaSpin->value());
        else if(ui->derivScharrRadio->isChecked())
            Scharr(t_source, t_destination, -1, 1, 0, ui->derivScaleSpin->value(), ui->derivDeltaSpin->value());
        else if(ui->derivLaplacRadio->isChecked())
            Laplacian(t_source, t_destination, -1, 3, ui->derivScaleSpin->value(), ui->derivDeltaSpin->value());
        break;
