...
void Keypoint_Plugin::setupUi(QWidget *parent)
{
    ui = new Ui::PluginGui;
    ui->setupUi(parent);
    m_agast_type_list << cv::AgastFeatureDetector::DetectorType::AGAST_5_8
                      << cv::AgastFeatureDetector::DetectorType::AGAST_7_12d
                      << cv::AgastFeatureDetector::DetectorType::AGAST_7_12s
    ...
    ui->agastTypeCombo->addItems(QStringList()
                                 ...
                                 << "AGAST_7_12s"
                                 << "OAST_9_16");
    m_fast_type_list << cv::FastFeatureDetector::DetectorType::TYPE_5_8
                     << cv::FastFeatureDetector::DetectorType::TYPE_7_12
                     << cv::FastFeatureDetector::DetectorType::TYPE_9_16;
    ui->fastTypeCombo->addItems(QStringList()
                                << "TYPE_5_8"
                                << "TYPE_7_12"
                                << "TYPE_9_16");
    m_akaze_type_list << cv::AKAZE::DescriptorType::DESCRIPTOR_KAZE_UPRIGHT
                      << cv::AKAZE::DescriptorType::DESCRIPTOR_KAZE
                      << cv::AKAZE::DescriptorType::DESCRIPTOR_MLDB_UPRIGHT
                      << cv::AKAZE::DescriptorType::DESCRIPTOR_MLDB;
    ui->akazeDescriptCombo->addItems(QStringList()
                                     << "DESCRIPTOR_KAZE_UPRIGHT"
                                     << "DESCRIPTOR_KAZE"
                                     << "DESCRIPTOR_MLDB_UPRIGHT"
                                     << "DESCRIPTOR_MLDB");
    m_kaze_type_list << cv::KAZE::DiffusivityType::DIFF_PM_G1
                     << cv::KAZE::DiffusivityType::DIFF_PM_G2
                     << cv::KAZE::DiffusivityType::DIFF_WEICKERT
                     << cv::KAZE::DiffusivityType::DIFF_CHARBONNIER;
    ...
    ui->descriptorCombo->addItems(QStringList()
                                  ...
                                  << "BRISK"
                                  << "ORB");
    m_matcher_type << cv::DescriptorMatcher::MatcherType::FLANNBASED
                   ...
                   << cv::DescriptorMatcher::MatcherType::BRUTEFORCE_HAMMINGLUT
                   << cv::DescriptorMatcher::MatcherType::BRUTEFORCE_SL2;
...
void Keypoint_Plugin::processImage(const cv::Mat &inputImage, cv::Mat &outputImage)
{
    ...
    descMather = DescriptorMatcher::create(m_matcher_type.at(ui->matcherCombo->currentIndex()));
...
void Keypoint_Plugin::fillFeature2D(QString algName, cv::Ptr<cv::Feature2D> &algorithm)
{
    ...
    if(algName == "AGAST")
    {
        ...
        agast->setType(m_agast_type_list.at(ui->agastTypeCombo->currentIndex()));
        algorithm = agast;
    }
    else if(algName == "KAZE")
    {
        if(ui->kazeAcceleratedCheck->isChecked())
        {
            ...
            akaze->setDescriptorType(m_akaze_type_list.at(ui->akazeDescriptCombo->currentIndex()));
            akaze->setDiffusivity(m_kaze_type_list.at(ui->kazeDiffCombo->currentIndex()));
        ...
        else
        {
            ...
            kaze->setDiffusivity(m_kaze_type_list.at(ui->kazeDiffCombo->currentIndex()));
    ...
    else if(algName == "FAST")
    {
        ...
        fast->setType(m_fast_type_list.at(ui->fastTypeCombo->currentIndex()));
        algorithm = fast;
    }
